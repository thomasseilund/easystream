...
CAM0="-f v4l2 ${THREAD_QUEUE_SIZE} -input_format mjpeg -s ${FSIZE} -r 30 -i /dev/video${DEV_CAM0}"
CAM1="-f v4l2 ${THREAD_QUEUE_SIZE} -input_format mjpeg -s ${FSIZE} -r 15 -i /dev/video${DEV_CAM1}"
...
function help {
	...
	echo "Call $(basename $0)t"
	echo "Ie. \`es2_capture.sh\`"
	echo "-f script file to ffmpeg option -filter_complex_script. Default filter.d/filter"
...
ffmpeg -y -f lavfi -i color=c=red:size=$FSIZE -frames:v 25 -pix_fmt $PIX_FMT -f mjpeg ./playbackFiller/25.mjpeg
ffmpeg -y -f lavfi -i color=c=red:size=$FSIZE -frames:v  1 -pix_fmt $PIX_FMT -f mjpeg ./playbackFiller/1.mjpeg

#
# Sæt manuel exposure for Logitech C920, så fps er konstant uanset lysstyrke!
#

v4l2-ctl -d ${DEV_CAM0} --set-ctrl=exposure_auto=1

#
# Call ffmpeg with piped input from playback program
#
